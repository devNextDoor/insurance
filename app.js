...
app.post("/index", function(req, res) {
    ...
    Insurance.create(req.body.insurance, function(err, newInsurance){
        if(err){
            ...
            console.log(typ);
            res.redirect("/"+typ.charAt(0).toLowerCase()+typ.substring(1,typ.length)+"db");
        }
    });
});
...
app.get("/healthdb",function(req, res){
    Insurance.find({type : "Health"}, function(err, insurances) {
        if(err){
            ...
            res.render("healthdb", {insurances : insurances});
        }
    });
});
    ...
    User.find({username : currentUser}, function(err, foundUser) {
        if(err){
            ...
            if(foundUser.length>0){
                ...
                console.log(foundUser[0].isAdmin);
                ...
                Buy.find({username : currentUser}, function(err, foundBuy) {
                    if(err){
                        ...
                        //console.log(foundBuy[0].buys);
                        console.log("Hello111111");
                        console.log(currentUser);
                        
                        res.render("user_home",{user : foundBuy, un : currentUser});
                        buy = false;
                    }
                });
            }
...
app.post("/register",function(req, res){
    var obj = {fullname : req.body.fullname, username : req.body.username, email : req.body.email};
    User.register(new User(obj), req.body.password, function(err, user){
        if(err){
            console.log(err);
            console.log("Register ni hua");
           ...
           Buy.find({username :""+ currentUser}, function(err, foundBuy) {
               if(err){
                   ...
                   if(foundBuy.length==0){
                       console.log("Inside First time Buyer");
